              ...
              {items.map((item) => (
                <div key={item.id} className={`group bg-card rounded-xl border border-border p-5 transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 ${accentColor}`}>
                  <div className="flex items-center justify-between mb-3">
                    <div className={`w-9 h-9 rounded-lg bg-blue-100  ${iconBg} flex items-center justify-center`}>
                  {icon}
                    </div>
                  ...
                  <h3 className="text-sm font-semibold text-card-foreground mb-1">{item.name}</h3>
